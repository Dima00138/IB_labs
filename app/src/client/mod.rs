    ...
    C: Has<XSpanIdString> + Has<Option<AuthData>> + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<MessageGetResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<Vec<models::Message>>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(MessageGetResponse::OK
                    (body)
                )
            }
            404 => {
        ...
        context: &C) -> Result<MessagePostResponse, ApiError>
    {
        ...
        let mut uri = format!(
            "{}/v1/message",
            self.base_path
            ,body=utf8_percent_encode(&param_body.to_string(), ID_ENCODE_SET)
        );
        ...
        context: &C) -> Result<MessagePutResponse, ApiError>
    {
        ...
        let query_string = {
            let mut query_string = form_urlencoded::Serializer::new("".to_owned());
                query_string.append_pair("MessageId",
                    &param_message_id.to_string());
            query_string.finish()
        };
        if !query_string.is_empty() {
            uri += "?";
